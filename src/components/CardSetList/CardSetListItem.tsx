...
const CardSetListItem = ({ set, cardSets }: { set: CardSetType, cardSets: CardSetType[] }) => {
	...
	const editCardSetHandler = async (isEdit: boolean) => {
		...
		if (isEdit && nameValue.trim().length > 0 && !(nameValue.trim() === currName && descriptionValue.trim() === currDescription)) {
			...
			if (status === 204) {
				...
				const newSet = { set_id: set.set_id, name: nameValue.trim(), description: descriptionValue.trim(), numCards: set.numCards };
